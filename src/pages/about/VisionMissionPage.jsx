...
export default function VisionMissionPage() {
  ...
  const visionMission = content?.about?.visionMission;

  return (
    <PageWrapper 
      title={visionMission?.page?.title?.[lang] || (lang === 'en' ? 'Vision & Mission' : 'লক্ষ্য ও উদ্দেশ্য')}
      subtitle={visionMission?.page?.subtitle?.[lang] || (lang === 'en' ? 'Our guiding principles for creating an inclusive society.' : 'একটি অন্তর্ভুক্তিমূলক সমাজ গঠনের জন্য আমাদের পথপ্রদর্শক নীতিসমূহ।')}
    >
      <div className="space-y-32">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-0">
            ...
            >
              ...
              <h3 className="text-4xl lg:text-5xl font-display font-bold mb-8">
                {visionMission?.vision?.title?.[lang] || (lang === 'en' ? 'Our Vision' : 'আমাদের লক্ষ্য')}
              </h3>
              <p className="text-white/80 leading-relaxed text-xl lg:text-2xl italic font-serif">
                "{visionMission?.vision?.text?.[lang] || (lang === 'en' 
            ...
            >
              ...
              <h3 className="text-4xl lg:text-5xl font-display font-bold mb-8">
                {visionMission?.mission?.title?.[lang] || (lang === 'en' ? 'Our Mission' : 'আমাদের উদ্দেশ্য')}
              </h3>
              <p className="text-white/80 leading-relaxed text-xl lg:text-2xl">
                {visionMission?.mission?.text?.[lang] || (lang === 'en' 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-6">
              <h2 className="text-4xl lg:text-5xl font-display font-bold text-text-main">
                {visionMission?.values?.title?.[lang] || (lang === 'en' ? 'Our Core Values' : 'আমাদের মূল মূল্যবোধ')}
              </h2>
              <p className="text-muted text-lg leading-relaxed">
                {visionMission?.values?.desc?.[lang] || (lang === 'en' 
              ...
              <div className="grid grid-cols-2 gap-6">
                {(visionMission?.values?.list || [
                  ...
                  <div key={idx} className="flex items-center gap-4">
                    ...
                    <span className="font-bold text-text-main text-lg">{val?.[lang] || val?.en}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="text-center space-y-8 py-12">
          <h2 className="text-3xl lg:text-4xl font-display font-bold text-text-main">
            {visionMission?.join?.title?.[lang] || (lang === 'en' ? 'Join Us in Our Mission' : 'আমাদের মিশনে যোগ দিন')}
          </h2>
          <div className="flex flex-wrap justify-center gap-6">
            <button className="px-10 py-5 bg-primary text-white rounded-full font-bold text-lg shadow-xl hover:bg-primary/90 transition-all">
              {visionMission?.join?.volunteer?.[lang] || (lang === 'en' ? 'Become a Volunteer' : 'স্বেচ্ছাসেবী হন')}
            </button>
            <button className="px-10 py-5 bg-white text-text-main border border-border rounded-full font-bold text-lg shadow-lg hover:bg-surface-alt transition-all">
              {visionMission?.join?.donate?.[lang] || (lang === 'en' ? 'Donate Now' : 'এখনই দান করুন')}
            </button>
          </div>
        </section>
      </div>
